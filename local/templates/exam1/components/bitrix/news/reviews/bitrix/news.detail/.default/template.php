...
<? if ($arResult["PROPERTIES"]["FILES"]["VALUE"]) { ?>
	<div class="exam-review-doc">
		...
		<? foreach ($arResult["PROPERTIES"]["FILES"]["VALUE"] as $file) {
			$fileSrc = CFILE::GetPath($file);
			$fileOb = CFILE::GetByID($file);
			$fileOb = $fileOb->fetch();
		?>
		<div  class="exam-review-item-doc"><img class="rew-doc-ico" src="<?= SITE_TEMPLATE_PATH ?>/img/icons/pdf_ico_40.png"><a href="<?= $fileSrc ?>"><?= $fileOb["ORIGINAL_NAME"] ?></a></div>
		<? } ?>
	</div>
<? } ?>
<hr>
